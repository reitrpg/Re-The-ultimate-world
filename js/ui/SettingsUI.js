...
class SettingsUI {
    ...
    registerControls() {
        ...
        if (tickSpeed) {
            tickSpeed.addEventListener("change", event => {
                ...
                SettingsManager.set("tickSpeed", value);
                Game.restart();
            });
        }
    ...
    render() {
        ...
        const numberFormat = this.getElement(
            "settings-number-format"
        );
        const tickSpeed = this.getElement(
            "settings-tick-speed"
        );
        const autoSave = this.getElement(
            "settings-autosave"
        );
        const debugMode = this.getElement(
            "settings-debug"
        );
        const speedRunMode = this.getElement(
            "settings-speedrun"
        );
        const language = this.getElement(
            "settings-language"
        );
        ...
        if (tickSpeed) {
            tickSpeed.value = settings.tickSpeed;
        }

        if (autoSave) {
            autoSave.value = settings.autoSaveInterval;
        }

        if (debugMode) {
            debugMode.checked = Boolean(settings.debugMode);
        }

        if (speedRunMode) {
            speedRunMode.checked = Boolean(settings.speedRunMode);
        }
